    ...
    def send_pos_ordermail(self, **kw):
        ...
        for order in orders:
            if order.reserved and order.session_id.config_id.enable_pos_welcome_mail:
                try:
                    template_id = request.env['ir.model.data'].get_object_reference('flexiretail_ee_advance', 'email_template_pos_ereceipt_reservation')
            ...
            if order.partner_id.email and order.session_id.config_id.enable_ereceipt and not order.reserved:
                try:
                    template_id = request.env['ir.model.data'].get_object_reference('flexiretail_ee_advance', 'email_template_pos_ereceipt')
                    template_obj = request.env['mail.template'].browse(template_id[1])
...
class PosMirrorController(http.Controller):
    ...
    def white_board_web(self, **k):
        ...
        return request.render('flexiretail_ee_advance.customer_display_index', qcontext=context)
