...
class wizard_sales_details(models.TransientModel):
    ...
    def print_sales_details(self):
        ...
        return self.env.ref('flexiretail_ee_advance.report_sales_details_pdf').report_action(self, data=datas)
    ...
    def get_total_sales(self, user_lst=None):
        if self:
            ...
            first_date, last_date = self.start_end_date_global(self.start_date, self.end_date, self.env['res.users'].browse([self._uid]).tz)
