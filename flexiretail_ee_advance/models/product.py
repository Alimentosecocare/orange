        ...
        if res:
            ...
            if not vals.get('barcode') and gen_barcode:
                ...
                if barcode_selection == 'ean_13':
                    barcode_code = barcode.ean.EuropeanArticleNumber13(str(res.id) + datetime.now().strftime("%S%M%H%d%m%y"))
                ...
                if barcode_selection == 'isbn_13':
                    barcode_code = barcode.isxn.InternationalStandardBookNumber13('978' + str(res.id) + datetime.now().strftime("%S%M%H%d%m%y"))
                if barcode_selection == 'isbn_10':
                    barcode_code = barcode.isxn.InternationalStandardBookNumber10(str(res.id) + datetime.now().strftime("%S%M%H%d%m%y"))
                if barcode_selection == 'issn':
                    barcode_code = barcode.isxn.InternationalStandardSerialNumber(str(res.id) + datetime.now().strftime("%S%M%H%d%m%y"))
...
class ProductProduct(models.Model):
    ...
    def expiry_product_alert(self):
        ...
        if notification_days:
            ...
            email_list = []
            template_id = self.env.ref('flexiretail_ee_advance.email_template_product_expiry_alert')
