    ...
    def generate_barcode(self):
        for rec in self.env['product.product'].browse(self._context.get('active_ids')):
            ...
            if self.barcode_selection == 'ean_13':
                barcode_code = barcode.ean.EuropeanArticleNumber13(str(rec.id) + datetime.now().strftime("%S%M%H%d%m%y"))
            ...
            if self.barcode_selection == 'isbn_13':
                barcode_code = barcode.isxn.InternationalStandardBookNumber13('978' + str(rec.id) + datetime.now().strftime("%S%M%H%d%m%y"))
            if self.barcode_selection == 'isbn_10':
                barcode_code = barcode.isxn.InternationalStandardBookNumber10(str(rec.id) + datetime.now().strftime("%S%M%H%d%m%y"))
            if self.barcode_selection == 'issn':
                barcode_code = barcode.isxn.InternationalStandardSerialNumber(str(rec.id) + datetime.now().strftime("%S%M%H%d%m%y"))
