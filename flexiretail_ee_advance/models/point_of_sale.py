...
class pos_order(models.Model):
    ...
    def check_order_delivery_type(self):
        if self.delivery_type == 'pending' and self.state == 'draft':
            action_id = self.env.ref('flexiretail_ee_advance.action_pos_payment_flexiretail')
            return {
                'name': action_id.name,
                'type': action_id.type,
    ...
    def load_ir_config_parameter(self):
        record = {}
        sys_param_goole_key = self.env['ir.config_parameter'].sudo().search([('key','=','google_api_key')],order='id desc',limit=1)
            ...
            if order.get('giftcard'):
                for create_details in order.get('giftcard'):
                    vals = {
                        ...
                        'expire_date':datetime.strptime(create_details.get('giftcard_expire_date'),'%Y/%m/%d').strftime('%Y-%m-%d'),
...
#                                 qty = move.product_uom_qty
    ...
    def send_reserve_mail(self):
        if self and self.customer_email and self.reserved and self.fresh_order:
            try:
                template_id = self.env['ir.model.data'].get_object_reference('flexiretail_ee_advance', 'email_template_pos_ereceipt')
...
#                        date1 = datetime.strptime(order_date, '%Y-%m-%d  %H:%M:%S')
    ...
    rating = fields.Selection([('0', 'Bad'), ('1', 'Not bad'), ('2', 'Normal'), ('3', 'Good'),('4', 'Very Good'),('5', 'Excellent')], 'Rating')
...
class pos_session(models.Model):
    ...
    def get_products_category_data(self,flag_config):
        ...
        product_records = self.env['product.product'].sudo().with_context({'location':self.config_id.stock_location_id.id}).search(domain).ids
...
#                 total_discount += sum([((line.qty * line.price_unit) * line.discount) / 100 for line in order.lines])
    ...
    def get_total_first(self):
        total = 0.0
        if self:
            total = ((self.get_total_sales() + self.get_total_tax() + self.get_money_in_total() + self.cash_register_balance_start) + self.get_money_out_total())\
...
#                 tz = timezone(tz)
    ...
    def get_total_first_x(self):
        global gross_total
        if self:
            gross_total = ((self.get_total_sales() + self.get_total_tax() + self.get_money_in_total() + self.cash_register_balance_start) + self.get_money_out_total()) \
