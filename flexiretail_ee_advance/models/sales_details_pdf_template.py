...
class sales_details_pdf_template(models.AbstractModel):
    _name = 'report.flexiretail_ee_advance.sales_details_pdf_template'
    _description = 'Used to Store Value for Sales Detail PDF Report.'

    @api.model
    def _get_report_values(self, docids, data=None):
        report = self.env['ir.actions.report']. \
            _get_report_from_name('flexiretail_ee_advance.sales_details_pdf_template')
    ...
    #     report = report_obj._get_report_from_name('flexiretail_ee_advance.sales_details_pdf_template')
    #     docargs = {
    ...
    #         'data': data
    #     }
    #     return report_obj.render('flexiretail_ee_advance.sales_details_pdf_template', docargs)

class pos_sales_report_pdf_template(models.AbstractModel):
    _name = 'report.flexiretail_ee_advance.pos_sales_report_pdf_template'
    _description = 'Used to Store Value for POS Sales Report PDF.'

    @api.model
    def _get_report_values(self, docids, data=None):
        report = self.env['ir.actions.report'].\
            _get_report_from_name('flexiretail_ee_advance.pos_sales_report_pdf_template')
